...
export default function CartDrawer() {
  ...
  return <AnimatePresence>{open && <>
    ...
    <motion.aside key="cart-drawer" role="dialog" aria-modal="true" aria-label="Carrito de compras" className="fixed bottom-0 right-0 top-0 z-50 flex w-full max-w-[460px] flex-col bg-[var(--surface)] shadow-[-20px_0_60px_rgba(0,0,0,0.18)]" initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ type: "spring", damping: 30, stiffness: 280 }}>
      ...
      {items.length > 0 && <footer className="border-t border-[var(--border)] bg-[var(--surface)] p-5"><div className="space-y-2 text-[13px]"><div className="flex justify-between text-[var(--text-secondary)]"><span>Base 0%</span><span>{formatCurrency(totals.subtotalBase0)}</span></div><div className="flex justify-between text-[var(--text-secondary)]"><span>Base IVA {ivaRate}%</span><span>{formatCurrency(totals.subtotalBase15)}</span></div><div className="flex justify-between text-[var(--text-secondary)]"><span>IVA {ivaRate}%</span><span>{formatCurrency(totals.ivaAmount)}</span></div><div className="mt-3 flex justify-between border-t border-[var(--border)] pt-3 text-[18px] font-bold"><span>Total</span><span>{formatCurrency(totals.total)}</span></div></div><Link href="/checkout" onClick={closeCart} className="btn-primary mt-5 flex w-full justify-center">Proceder al pago seguro</Link></footer>}
    </motion.aside>
  </>}</AnimatePresence>;
}
