  ...
  return (
    <section className="relative w-full overflow-hidden py-6" aria-label="Pasarela de fundas y protección">
      <div className="relative w-full conveyor-track">
        ...
        <div className="pointer-events-none absolute inset-y-0 left-0 z-20 w-10 bg-linear-to-r from-(--bg-secondary) via-(--bg-secondary)/85 to-transparent sm:w-24" aria-hidden="true" />

        {/* Portal Derecho — pegado al borde derecho de pantalla */}
        <div className="pointer-events-none absolute inset-y-0 right-0 z-20 w-10 bg-linear-to-l from-(--bg-secondary) via-(--bg-secondary)/85 to-transparent sm:w-24" aria-hidden="true" />
        ...
        <div className="animate-conveyor py-2">
          {conveyorItems.map((funda, index) => (
            <article
              key={`${funda.id}-${index}`}
              className="group mr-4 w-55 shrink-0 rounded-lg border border-border bg-surface p-4 shadow-(--shadow-sm) transition-transform duration-300 hover:-translate-y-1 hover:shadow-(--shadow-lg) sm:w-62.5"
            >
              <div className="relative mb-3 flex h-32 items-center justify-center overflow-hidden rounded-md bg-[linear-gradient(145deg,var(--bg-secondary),var(--surface))]">
                {funda.image_url ? (
                  <Image
                    src={funda.image_url}
              ...
              <div className="mt-4 flex items-center justify-between border-t border-border pt-3">
                ...
                <a
                  ...
                  className="inline-flex items-center gap-1.5 rounded-full bg-accent px-3.5 py-2 text-[13px] font-semibold text-white transition-all duration-150 hover:bg-accent-hover active:scale-95"
                >
                  Pedir Funda
