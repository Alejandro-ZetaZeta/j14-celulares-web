...
export async function getAllProducts() {
  ...
  const { data, error } = await db
    .from("products")
    .select(`id, brand, model, type, image_url, image_key, allows_installments, is_featured, featured_order, featured_eyebrow, featured_headline, featured_description, featured_cta, created_at, product_images(id, product_id, image_url, image_key, display_order, created_at), product_variants(id, capacity, color, price, stock, battery_condition)`)
...
export async function getProductForAdmin(id: string) {
  ...
  const { data, error } = await db
    .from("products")
    .select(`id, brand, model, type, image_url, image_key, allows_installments, is_featured, featured_order, featured_eyebrow, featured_headline, featured_description, featured_cta, created_at, product_images(id, product_id, image_url, image_key, display_order, created_at), product_variants(id, product_id, capacity, color, price, stock, battery_condition, created_at)`)
    .eq("id", id)
    .single();
