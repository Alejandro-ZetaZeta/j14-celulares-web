...
import type { AdminOrder, AdminOrderDetail, OrderStatus } from "@/types/database";
...
export default function OrderDetailDrawer({ order, onClose, onUpdated }: { order: AdminOrder | null; onClose: () => void; onUpdated: (order: AdminOrder) => void }) {
  ...
  useEffect(() => {
    ...
    document.title = `NotaVenta-${order.id}`;
    return () => {
      document.title = prev;
    };
  }, [order]);
  ...
  const rawPhone = (current.customer.phone ?? "").replace(/\D/g, "");
  const whatsapp = rawPhone.length === 10 && rawPhone.startsWith("0") ? `593${rawPhone.slice(1)}` : rawPhone;

  function printReceipt() {
    const prev = document.title;
    document.title = `NotaVenta-${selectedOrder.id}`;
