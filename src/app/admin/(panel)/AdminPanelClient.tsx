...
const allSidebarLinks = [
  { href: "/admin", label: "Dashboard", icon: IconDashboard, exact: true, roles: ["admin"] as AppRole[] },
  { href: "/admin/productos", label: "Productos", icon: IconPackage, roles: ["admin"] as AppRole[] },
  ...
  { href: "/admin/ventas", label: "Ventas", icon: IconReceipt, roles: ["admin"] as AppRole[] },
  {
    ...
    children: [{ href: "/admin/servicio-tecnico/configuracion", label: "Personalizar página", icon: IconPen }],
  },
];
    ...
    <div className="admin-panel-shell flex h-screen max-h-screen h-dvh max-h-dvh overflow-hidden bg-[var(--bg-secondary)]">
      ...
      <aside className={`admin-sidebar bg-[var(--bg-dark)] flex flex-col flex-shrink-0 h-full max-h-full overflow-y-auto ${isSidebarCollapsed ? "is-collapsed" : ""}`}>
        ...
        <nav className="flex-1 px-3 py-4" aria-label="Navegación del panel">
          ...
          <ul className="space-y-0.5">
            {sidebarLinks.map(({ href, label, icon, exact, children }) => (
              <li key={href}>
                <div className={`admin-sidebar__nav-group flex items-center ${children ? "has-submenu" : ""} ${children && isActive(href, exact) ? "is-active" : ""}`}>
                  <Link
                    href={href}
                  ...
                  >
                    <span className="admin-sidebar__icon" aria-hidden="true">{icon ? icon({ className: "admin-sidebar__icon-svg" }) : null}</span>
        ...
        <div className="admin-sidebar__footer px-3 py-4 border-t border-white/10 space-y-1">
          ...
          >
            <span aria-hidden="true"><IconGlobe className="admin-sidebar__icon-svg" /></span>
          ...
          >
            <span aria-hidden="true"><IconLogout className="admin-sidebar__icon-svg" /></span>
