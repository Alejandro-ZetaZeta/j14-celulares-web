      ...
      <div className="flex flex-col gap-4">
        ...
        <form onSubmit={submit} className="flex flex-col gap-3.5">
          <div className="grid gap-3.5 sm:grid-cols-2">
           <label className="flex flex-col gap-1.5 text-[13px] font-semibold">Nombre completo<input required maxLength={100} value={fullName} onChange={(e) => setFullName(e.target.value)} autoComplete="name" className="rounded-[11px] border border-[var(--border-strong)] px-4 py-2.5 text-[14px] outline-none focus:border-[var(--accent)] focus:ring-2 focus:ring-[var(--accent)]/20" /></label>
           <label className="flex flex-col gap-1.5 text-[13px] font-semibold">Correo electrónico<input required type="email" maxLength={254} value={email} onChange={(e) => setEmail(e.target.value)} autoComplete="email" className="rounded-[11px] border border-[var(--border-strong)] px-4 py-2.5 text-[14px] outline-none focus:border-[var(--accent)] focus:ring-2 focus:ring-[var(--accent)]/20" /></label>
           <label className="flex flex-col gap-1.5 text-[13px] font-semibold">Teléfono<input required maxLength={10} inputMode="numeric" value={phone} onChange={(e) => setPhone(e.target.value.replace(/\D/g, "").slice(0, 10))} autoComplete="tel" className="rounded-[11px] border border-[var(--border-strong)] px-4 py-2.5 text-[14px] outline-none focus:border-[var(--accent)] focus:ring-2 focus:ring-[var(--accent)]/20" /></label>
           <label className="flex flex-col gap-1.5 text-[13px] font-semibold">Contraseña<span className="relative"><input required type={showPassword ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} autoComplete="new-password" className="w-full rounded-[11px] border border-[var(--border-strong)] px-4 py-2.5 pr-12 text-[14px] outline-none focus:border-[var(--accent)] focus:ring-2 focus:ring-[var(--accent)]/20" /><PasswordVisibilityButton visible={showPassword} onToggle={() => setShowPassword((visible) => !visible)} /></span></label>
          </div>
          <ul aria-label="Requisitos de contraseña" className="grid grid-cols-1 gap-1 rounded-[12px] bg-[var(--bg-secondary)] p-2.5 text-[11px] text-[var(--text-secondary)] sm:grid-cols-2 lg:grid-cols-4">
