...
export default function CompleteProfileForm({ initial }: { initial: { cedula: string; dateOfBirth: string; address: string; province: string; city: string; postcode: string } }) {
    ...
    <form onSubmit={submit} aria-busy={saving} className="flex flex-col gap-4">
      ...
      <div className="grid gap-3.5 sm:grid-cols-2">
       ...
       <label className="flex flex-col gap-1.5 text-[13px] font-semibold">Fecha de nacimiento <span className="font-normal text-[var(--text-tertiary)]">Mayor de 18 años</span><input required id="date_of_birth" name="date_of_birth" type="date" max={new Date(new Date().setFullYear(new Date().getFullYear() - 18)).toISOString().slice(0, 10)} value={dateOfBirth} onChange={(e) => setDateOfBirth(e.target.value)} className="rounded-[11px] border border-[var(--border-strong)] px-4 py-2.5 text-[14px] outline-none focus:border-[var(--accent)] focus:ring-2 focus:ring-[var(--accent)]/20" /></label>
        ...
        <label className="flex flex-col gap-1.5 text-[13px] font-semibold">Código postal<span className="font-normal text-[var(--text-tertiary)]">Ejemplo: 010150</span><input required name="postcode" inputMode="numeric" maxLength={10} value={postcode} onChange={(e) => setPostcode(e.target.value.replace(/\D/g, "").slice(0, 10))} className="rounded-[11px] border border-[var(--border-strong)] px-4 py-2.5 text-[14px] outline-none focus:border-[var(--accent)] focus:ring-2 focus:ring-[var(--accent)]/20" /></label>
        <button type="submit" disabled={saving || !/^\d{1,10}$/.test(cedula) || !dateOfBirth || !address.trim() || !province || !city || !postcode} className="btn-primary w-full justify-center disabled:opacity-50">{saving ? "Guardando..." : "Completar perfil"}</button>
    </form>
  );
}
