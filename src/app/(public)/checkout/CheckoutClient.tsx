...
export default function CheckoutClient({ initialIsAuthenticated = false }: { initialIsAuthenticated?: boolean }) {
  ...
  async function proceed() {
    ...
    try {
      ...
      const response = await fetch("/api/payments/dataweb-checkout", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ customer, items: payloadItems, totals, promotionCode }) });
  ...
  return <main className="container-wide px-5 pb-24 pt-28"><div className="mb-10 max-w-2xl"><p className="catalog-kicker">Checkout seguro · Dataweb Sandbox</p><h1 className="mt-2 text-display">Completa tu compra</h1><p className="mt-3 text-[15px] text-[var(--text-secondary)]">Tu tarjeta se procesa en formulario seguro certificado de Datafast.</p></div><div className="grid gap-10 lg:grid-cols-[minmax(0,1fr)_360px]"><aside className="order-first h-fit rounded-[var(--radius-lg)] border border-[var(--border)] bg-[var(--bg-secondary)] p-5 sm:p-6 lg:order-2"><h2 className="text-[18px] font-bold">Resumen de compra</h2><div className="mt-5 space-y-4">{items.map((item) => <div key={item.variantId} className="flex justify-between gap-4 text-[13px]"><div><p className="font-semibold">{item.brand} {item.model}</p><p className="text-[var(--text-secondary)]">{item.capacity} · {item.color} · x{item.quantity}</p>{item.gifts.length > 0 && item.gifts.map((gift) => <p key={gift.variantId} className="mt-0.5 text-[12px] text-[var(--accent)]">🎁 Incluye regalo: {gift.brand} {gift.model} ({gift.capacity} · {gift.color}) x {gift.quantity * item.quantity}</p>)}</div><span className="shrink-0 font-semibold">{formatCurrency(item.unitPrice * item.quantity)}</span></div>)}</div><div className="mt-6 space-y-2 border-t border-[var(--border)] pt-5 text-[13px]"><div className="flex justify-between text-[var(--text-secondary)]"><span>Base IVA {ivaRate}%</span><span>{formatCurrency(totals.subtotalBase15)}</span></div><div className="flex justify-between text-[var(--text-secondary)]"><span>IVA {ivaRate}%</span><span>{formatCurrency(totals.ivaAmount)}</span></div><div className="flex justify-between border-t border-[var(--border)] pt-4 text-[20px] font-bold"><span>Total</span><span>{formatCurrency(totals.total)}</span></div></div><p className="mt-5 text-[11px] leading-4 text-[var(--text-tertiary)]">Ambiente Sandbox. No se realizara cargo real.</p></aside><section className="order-2 rounded-[var(--radius-lg)] border border-[var(--border)] bg-[var(--surface)] p-5 sm:p-8 lg:order-1"><h2 className="text-[20px] font-bold">Datos del cliente</h2><div className="mt-6 grid gap-5 sm:grid-cols-2"><label className="text-[13px] font-semibold text-[var(--text-secondary)]">Nombre completo *<input type="text" value={customer.fullName} onChange={(event) => update("fullName", event.target.value)} className="input-apple mt-2 w-full" required /></label><label className="text-[13px] font-semibold text-[var(--text-secondary)]">Cedula *<input type="text" inputMode="numeric" maxLength={10} value={customer.cedula} onChange={(event) => update("cedula", event.target.value.replace(/\D/g, "").slice(0, 10))} className="input-apple mt-2 w-full" required /></label><label className="text-[13px] font-semibold text-[var(--text-secondary)]">Correo electronico *<input type="email" value={customer.email} onChange={(event) => update("email", event.target.value)} className="input-apple mt-2 w-full" required /></label><label className="text-[13px] font-semibold text-[var(--text-secondary)]">Telefono *<input type="tel" value={customer.phone} onChange={(event) => update("phone", event.target.value)} className="input-apple mt-2 w-full" required /></label><label className="text-[13px] font-semibold text-[var(--text-secondary)] sm:col-span-2">Direccion de facturacion y envio *<input type="text" value={customer.address} onChange={(event) => update("address", event.target.value)} className="input-apple mt-2 w-full" required /></label></div></section><div className="order-3 flex flex-col gap-5">{!isAuthenticated && <label className="mt-6 flex items-start gap-3 text-[13px] leading-5 text-[var(--text-secondary)]"><input type="checkbox" checked={guestTermsAccepted} onChange={(event) => setGuestTermsAccepted(event.target.checked)} className="mt-0.5 shrink-0" /><span>Acepto <Link href="/terminos" className="underline hover:text-[var(--accent)]">Términos y Condiciones</Link> y <Link href="/privacidad" className="underline hover:text-[var(--accent)]">política de privacidad</Link>.</span></label>}{error && <p className="mt-5 rounded-[var(--radius-sm)] bg-red-50 px-4 py-3 text-[13px] text-red-700">{error}</p>}{status && <p className="mt-5 text-[13px] text-[var(--text-secondary)]">{status}</p>}{!checkoutId ? <button type="button" onClick={() => void proceed()} disabled={paying} className="btn-primary mt-7 w-full disabled:opacity-50">{paying ? "Preparando..." : "Continuar con Dataweb"}</button> : <form action="/api/payments/dataweb-result" className="paymentWidgets mt-7" data-brands="VISA MASTER DINERS DISCOVER AMEX" />}</div></div></main>;
}