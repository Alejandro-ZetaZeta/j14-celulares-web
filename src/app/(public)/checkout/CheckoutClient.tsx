...
export default function CheckoutClient({ initialIsAuthenticated = false }: { initialIsAuthenticated?: boolean }) {
  ...
  const { items, totals, ivaRate, clearCart, promotionCode } = useCart();
  ...
  async function proceed() {
    ...
    if (!customer.fullName.trim() || !/^\d{10}$/.test(customer.cedula.trim()) || !/^\S+@\S+\.\S+$/.test(customer.email.trim()) || !customer.phone.trim() || !customer.address.trim()) {
      setError("Completa todos los datos. El ID debe contener exactamente 10 dígitos.");
      return;
    }
    setError("");
    setStatus("");
      ...
      await openPaybox(checkoutTotals, checkoutCustomer, async (pagopluxResponse) => {
        try {
          setStatus("Confirmando tu pago...");
           const response = await fetch("/api/payments/pagoplux-callback", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ pagopluxResponse, customer: checkoutCustomer, items: checkoutItems, promotionCode }) });
  ...
  if (!items.length) return <main className="container-wide flex min-h-[65vh] items-center justify-center px-5 py-28"><div className="text-center"><p className="catalog-kicker">Checkout</p><h1 className="mt-2 text-display">Tu carrito está vacío</h1><Link href="/catalogo" className="btn-primary mt-6 inline-flex">Volver al catálogo</Link></div></main>;
    ...
    return <main className="container-wide px-5 pb-24 pt-28"><div className="mb-10 max-w-2xl"><p className="catalog-kicker">Checkout seguro · Sandbox</p><h1 className="text-display mt-2">Completa tu compra</h1><p className="mt-3 text-[15px] text-[var(--text-secondary)]">Ingresa tus datos para abrir el pago seguro de PagoPlux.</p></div><div className="grid gap-10 lg:grid-cols-[minmax(0,1fr)_360px]">
      <section className="rounded-[var(--radius-lg)] border border-[var(--border)] bg-[var(--surface)] p-5 sm:p-8"><h2 className="text-[20px] font-bold">Datos del cliente</h2><div className="mt-6 grid gap-5 sm:grid-cols-2"><label className="text-[13px] font-semibold text-[var(--text-secondary)]">Nombre completo <span className="text-[var(--status-red)]">*</span><input type="text" value={customer.fullName} onChange={(event) => update("fullName", event.target.value)} className="mt-2 h-11 w-full rounded-[var(--radius-sm)] border border-[var(--border-strong)] bg-transparent px-3 text-[14px] font-normal text-[var(--text-primary)] outline-none focus:border-[var(--accent)]" required /></label><label className="text-[13px] font-semibold text-[var(--text-secondary)]">Cédula <span className="text-[var(--status-red)]">*</span><input type="text" inputMode="numeric" pattern="\d*" maxLength={10} value={customer.cedula} onChange={(event) => update("cedula", event.target.value.replace(/\D/g, "").slice(0, 10))} className="mt-2 h-11 w-full rounded-[var(--radius-sm)] border border-[var(--border-strong)] bg-transparent px-3 text-[14px] font-normal text-[var(--text-primary)] outline-none focus:border-[var(--accent)]" required /></label><label className="text-[13px] font-semibold text-[var(--text-secondary)]">Correo electrónico <span className="text-[var(--status-red)]">*</span><input type="email" value={customer.email} onChange={(event) => update("email", event.target.value)} className="mt-2 h-11 w-full rounded-[var(--radius-sm)] border border-[var(--border-strong)] bg-transparent px-3 text-[14px] font-normal text-[var(--text-primary)] outline-none focus:border-[var(--accent)]" required /></label><label className="text-[13px] font-semibold text-[var(--text-secondary)]">Teléfono <span className="text-[var(--status-red)]">*</span><input type="tel" value={customer.phone} onChange={(event) => update("phone", event.target.value)} className="mt-2 h-11 w-full rounded-[var(--radius-sm)] border border-[var(--border-strong)] bg-transparent px-3 text-[14px] font-normal text-[var(--text-primary)] outline-none focus:border-[var(--accent)]" required /></label><label className="text-[13px] font-semibold text-[var(--text-secondary)] sm:col-span-2">Dirección <span className="text-[var(--status-red)]">*</span><input value={customer.address} onChange={(event) => update("address", event.target.value)} className="mt-2 h-11 w-full rounded-[var(--radius-sm)] border border-[var(--border-strong)] bg-transparent px-3 text-[14px] font-normal text-[var(--text-primary)] outline-none focus:border-[var(--accent)]" required /></label></div>{!isAuthenticated && <label className="mt-6 flex items-start gap-2 text-[13px] leading-5 text-[var(--text-secondary)]"><input type="checkbox" checked={guestTermsAccepted} onChange={(event) => setGuestTermsAccepted(event.target.checked)} className="mt-1 accent-[var(--accent)]" /><span>Acepto los <Link href="/terminos" className="font-semibold text-[var(--accent)] underline underline-offset-2">Términos y Condiciones</Link> y la política de devoluciones.</span></label>}{error && <p role="alert" className="mt-5 rounded-[var(--radius-sm)] bg-red-50 px-3 py-2 text-[13px] text-[var(--status-red)]">{error}</p>}{status && <p role="status" className="mt-5 rounded-[var(--radius-sm)] bg-[var(--bg-secondary)] px-3 py-2 text-[13px] text-[var(--text-secondary)]">{status}</p>}<button type="button" onClick={proceed} disabled={paying || (!isAuthenticated && !guestTermsAccepted)} className="btn-primary mt-7 flex w-full justify-center disabled:cursor-wait disabled:opacity-50">{paying ? "Abriendo PagoPlux..." : "Proceder al Pago con PagoPlux"}</button></section>
